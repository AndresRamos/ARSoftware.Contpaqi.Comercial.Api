...
public sealed class BuscarConceptosRequestHandler : IRequestHandler<BuscarConceptosRequest, ApiResponseBase>
{
    ...
    public async Task<ApiResponseBase> Handle(BuscarConceptosRequest request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                Concepto? concepto = await _conceptoRepository.BuscarPorIdAsync(request.Model.Id.Value, cancellationToken);
                if (concepto is not null)
                    conceptos.Add(concepto);
            }
            else if (request.Model.Codigo is not null)
            {
                Concepto? concepto = await _conceptoRepository.BuscarPorCodigoAsync(request.Model.Codigo, cancellationToken);
                if (concepto is not null)
                    conceptos.Add(concepto);
            }
            else
            {
                conceptos.AddRange(await _conceptoRepository.BuscarTodoAsync(cancellationToken));
            }
